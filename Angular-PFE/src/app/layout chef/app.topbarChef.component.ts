...
@Component({
    ...
    templateUrl: 'app.topbarChef.component.html'
})
    ...
    this.congeService.getNotificationDetailsCombined(userId).subscribe((data: any) => {
        ...
        Object.keys(data).forEach((type: string) => {
            ...
            if (notifications.length > 0) {
                ...
                notifications.forEach((notification: any) => {
                    if (notification.departement === this.departement || notification.departement === null) {
                        // Ajoutez une condition pour exclure les notifications ajoutées par l'utilisateur connecté
                        if (notification.note.includes(`ajouté par l'utilisateur ${this.name}`)) {
                            return; // Ignorez cette notification
                        }
                        const formattedDate = this.formatDate(notification.created_at);
                        dayLabel = this.isTodayOrYesterday(formattedDate);
                        notificationsGroupedByDate.push(notification);
